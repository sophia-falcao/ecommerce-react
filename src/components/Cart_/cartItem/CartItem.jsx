...
export function CartItem({data, setCartProduct}){

    const { removeItem, handleIncre, handleDecre} = useContext(ProducContext)


    return(
        <section className="cart_item" key={data._id}>  
          <img 
                src={data.image} 
            ...
            <div className="cart_item_content">
                ...
                <div className='container_btn_quantity'>
                    <button className='btn_quantity' onClick={() => handleDecre(data._id)}> - </button>
                    <p> {data.quantity} </p>
                    <button className='btn_quantity' onClick={() => handleIncre(data._id)}> + </button> 
                </div>
       
                <button type="button" className="btn_remove_item" onClick={() => removeItem(data._id, data.title)}>
                    <IoTrashOutline />
                </button>
            </div>   
        </section>
